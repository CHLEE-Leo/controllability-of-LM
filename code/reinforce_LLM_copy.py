...
for epoch in range(my_rl_epoch):
    ...
    for idx, (train_input_x_prefixed, train_input_att_prefixed, train_input_x) in enumerate(train_batch):        
        ...
        '''
        2-2) 타겟 모델의 생성 결과에 대해 행동 모델 logit값 계산
        '''
        outputs = gpt_model(padded_tp_gen_texts)
        output_logits = outputs.logits
        output_probs = tf.squeeze(tf.nn.softmax(output_logits, axis = -1))
        output_mean_probs = tf.math.reduce_mean(output_probs, axis =-1)
        output_sum_mean_probs = tf.reduce_sum(output_mean_probs)
        print('output_sum_mean_probs : {}'.format(output_sum_mean_probs))
